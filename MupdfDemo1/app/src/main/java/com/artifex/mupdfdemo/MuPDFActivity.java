...
public class MuPDFActivity extends Activity implements FilePicker.FilePickerSupport,Runnable,CallBack
{
					...
					if (!socket.isInputShutdown()) {
						if ((content = in.readLine()) != null) {
							...
							try{
								...
								if(mode==Annotation.Type.INK){
									JSONArray jsonArray=jsonObject.getJSONArray("path");
									int n=jsonObject.getInt("n");
									PointF path[][]=new PointF[n][]; //need to be determined
									for(int j=0;j<jsonArray.length();j++){
										JSONArray jsonArray1=jsonArray.getJSONArray(j);
										ArrayList<PointF> points = new ArrayList<PointF>();
										for(int i=0;i<jsonArray1.length();i++){
											JSONObject jsonObject2=new JSONObject(jsonArray1.get(i).toString());
											//path[j][i]=new PointF();
											points.add(new PointF(jsonObject2.getLong("x"),jsonObject2.getLong("y")));
										}
										path[j]=points.toArray(new PointF[points.size()]);
									}
									addDrawbymsg(path,page);
								}else{
	...
	public void addDrawbymsg(PointF path[][],int page){
		final MuPDFView pageView = (MuPDFView) mDocView.getDisplayedView();
		boolean success = false;
		if (pageView != null)
			success = pageView.saveDrawbymsg(page,path);
	}
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//					break;
	...
	public void OnAcceptButtonClick(View v) {
		...
		switch (mAcceptMode) {
			...
			case Ink:
				if (pageView != null)
					success = pageView.saveDraw(MuPDFActivity.this);
				mTopBarMode = TopBarMode.Annot;
				if (!success)
