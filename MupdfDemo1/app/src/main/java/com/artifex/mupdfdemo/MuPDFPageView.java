...
public class MuPDFPageView extends PageView implements MuPDFView{   ///////////////////////////////////////////////
	...
	///////////////////////////////////////////////////////////////////////////////////////////////
	public boolean saveDrawbymsg(final int page,final PointF path[][]) {
		if (path == null)
			return false;
		...
		mAddInk = new AsyncTask<PointF[][],Void,Void>() {
			@Override
			protected Void doInBackground(PointF[][]... params) {
				mCore.addInkAnnotation(page, params[0]);
				return null;
			}
			@Override
			protected void onPostExecute(Void result) {
	...
	public boolean saveDraw(final CallBack callBack) {
		PointF[][] path = getDraw();
		...
		mAddInk = new AsyncTask<PointF[][],Void,Void>() {
			...
			protected Void doInBackground(PointF[][]... params) {
				...
				try{
					jsonObj.put("page",mPageNumber);
					jsonObj.put("type", Annotation.Type.INK);
					jsonObj.put("n",params[0].length);
					JSONArray jsonArray=new JSONArray();
					for(int j=0;j<params[0].length;j++){
						JSONArray jsonArray1=new JSONArray();
						for(int i=0;i<params[0][j].length;i++){
							JSONObject jsonPoint=new JSONObject();
							jsonPoint.put("x",params[0][j][i].x);
							jsonPoint.put("y",params[0][j][i].y);
							jsonArray1.put(i,jsonPoint);
						}
//						JSONObject jsonObject=new JSONObject();
//						jsonObject.put("array",jsonArray1);
						jsonArray.put(j,jsonArray1);
					}
					jsonObj.put("path",jsonArray);
				}catch (JSONException e) {
					e.printStackTrace();
				}
				callBack.sendmsg(jsonObj);
